...
interface PlansPageProps {
  userPlan: string; // Define the userPlan prop
}

const PlansPage: React.FC<PlansPageProps> = ({ userPlan }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        {/* Conditional rendering based on the userPlan */}
        {userPlan !== "enterprise" && (
          <>
            {/* Free Plan */}
        ...
        {/* If the user already has an Enterprise Plan */}
        {userPlan === "enterprise" && (
          <div className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition duration-300 transform hover:scale-105">
            <h2 className="text-3xl font-bold text-red-600 mb-4">Enterprise Plan</h2>
            <p className="text-gray-600 mb-4">You are already subscribed to the Enterprise Plan with unlimited requests.</p>
          </div>
        )}
      </div>
    </div>
